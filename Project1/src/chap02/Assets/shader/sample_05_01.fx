...
float4 PSMain(SPSIn psIn) : SV_Target0
{
    ...
    const float3 distance = length(psIn.worldPos - ptPosition);
    ...
    const float3 diffuseLig = diffPoint + diffDirection;
    const float3 specularLig = specPoint + specDirection;

    // 拡散反射光と鏡面反射光を足し算して、最終的な光を求める
    const float3 lig = diffuseLig + specularLig + ambientLight;
