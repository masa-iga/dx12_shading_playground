...
{
public:
	void Init(RootSignature& rs, Light2* light, RenderTarget& depthRenderTarget);
	void Dispatch(RenderContext& renderContext, std::unique_ptr<Light2>& light);
...
private:
	...
	Zprepass2 m_zprepass;
	LightCulling2 m_lightCulling;
	std::unique_ptr<Model> m_modelTeapot = nullptr;
	std::unique_ptr<Model> m_modelBg = nullptr;
	RootSignature m_rootSignature;
	std::unique_ptr<Light2> m_light = nullptr;
};

void LightCulling2::Init(RootSignature& rs, Light2* light, RenderTarget& depthRenderTarget)
{
	m_rootSignature = &rs;
...

	{
		m_pointLightNoListInTileUAV.Init(
			sizeof(int32_t),
			static_cast<int32_t>(Light2::kNumPointLight * computeNumTile(depthRenderTarget)),
			nullptr);
	}
...
void LightCulling2::Dispatch(RenderContext& renderContext, std::unique_ptr<Light2>& light)
{
...
size_t LightCulling2::computeNumTile(const RenderTarget& depthRenderTarget) const
{
...
void Zprepass2::Draw(RenderContext& renderContext)
{
...
void Models_17_01::createModel()
{
	...
	m_light = std::make_unique<Light2>();
	{
